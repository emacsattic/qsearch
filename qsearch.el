...
;; Modified by Adrian Kubala to work with Xemacs
...
(defvar qsearch-mode-map
  (let ((mymap (make-keymap)))
    ...
    (mapc (lambda (key)
	    (mapc (lambda (modifier)
		    (define-key mymap (vector (list modifier key)) 'qsearch))
		  qsearch-modifiers))
	  (mapcar 'car qsearch-translations))
    (define-key mymap (kbd "C-s-s") 'qsearch-forward)
    (define-key mymap (kbd "C-H-s") 'qsearch-forward)
    (define-key mymap (kbd "C-s-r") 'qsearch-backward)
    (define-key mymap (kbd "C-H-r") 'qsearch-backward)
    mymap)
...
(defun qsearch-append-event ()
  ...
  (let ((keysym (event-key last-command-event))
	(modifiers (event-modifiers last-command-event)))
    (unless (characterp keysym)
      (setq keysym (cdr (assq keysym qsearch-translations))))
    (when (characterp keysym)
      (when (memq 'shift modifiers)
	(setq keysym (upcase keysym)))
      (setq qsearch-string
	(concat qsearch-string (string keysym)))
...
(easy-mmode-define-minor-mode qsearch-mode
